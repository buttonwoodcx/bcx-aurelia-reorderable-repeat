import {fireEvent, delay} from './utils';
...
test('reorderable-repeat: renders empty list', async t => {
...
test('reorderable-repeat: renders list', async t => {
  component = StageComponent
    .withResources(['reorderable-repeat'])
...
test('reorderable-repeat: re-rendering list should not cause memory leak', async t => {
...
test('reorderable-repeat: reorders', async t => {
  component = StageComponent
    .withResources(['reorderable-repeat'])
    .inView('<div style="height: 50px;w idth: 100px;" reorderable-repeat.for="obj of items">${obj.name}</div>')
    .boundTo({items: [{name: 'one'}, {name: 'two'}, {name: 'three'}]});
...
test('reorderable-repeat: reorders after array replacement', async t => {
  let model = {items: []};
  component = StageComponent
    .withResources(['reorderable-repeat'])
    .inView('<div style="height: 50px;w idth: 100px;" reorderable-repeat.for="obj of items">${obj.name}</div>')
...
test('reorderable-repeat: reorders duplicated strings', async t => {
  component = StageComponent
    .withResources(['reorderable-repeat'])
    .inView('<div style="height: 50px;w idth: 100px;" reorderable-repeat.for="obj of items">${obj.name}</div>')
